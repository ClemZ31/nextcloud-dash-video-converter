	...
	public function createCmd($file, $preset, $output, $priority, $movflags, $codec, $vbitrate, $scale)
	{
		...
		if ($output == "mpd") {
			...
			*/
			...
			$cmd = $changeDirCmd . $ffmepgPath . "ffmpeg -re -y -err_detect ignore_err -i " . escapeshellarg($file) . " -preset slow -keyint_min 100 -sc_threshold 0 -c:v libx264 -map v:0 -s:0 256x144 -b:v:0 160k -maxrate:0 160k -bufsize:0 320k -map v:0 -s:1 426x240 -b:v:1 400k -maxrate:1 400k -bufsize:1 800k -map v:0 -s:2 640x360 -b:v:2 700k -maxrate:2 700k -bufsize:2 1.4M -map v:0 -s:3 854x480 -b:v:3 1.25M -maxrate:3 1.25M -bufsize:3 2.5M -map v:0 -s:4 1280x720 -b:v:4 3.2M -maxrate:4 3.2M -bufsize:4 6.4M -map v:0 -s:5 1920x1080 -b:v:5 5.3M -maxrate:5 5.3M -bufsize:5 10.6M -map a:0 -c:a:0 aac -b:a:0 64k -ac 2 -map a:0 -c:a:1 aac -b:a:1 64k -ac 2 -map a:0 -c:a:2 aac -b:a:2 64k -ac 2 -map a:0 -c:a:3 aac -b:a:3 128K -ac 2 -map a:0 -c:a:4 aac -b:a:4 128K -ac 2 -map a:0 -c:a:5 aac -b:a:5 128K -ac 2 -f hls  -hls_list_size 0 -strftime_mkdir 1 -hls_flags split_by_time -hls_segment_type mpegts -hls_segment_filename '" . $hls_segment_filename . "' -master_pl_name '" . $master_pl_name . "' -var_stream_map 'v:0,a:0 v:1,a:1 v:2,a:2 v:3,a:3 v:4,a:4 v:5,a:5' '". $var_stream_map."'";
			$cmd .= " && " . $subTitlesConversionCmd;
			$cmd .= " && " . $refreshDirCmd;			
